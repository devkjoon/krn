const { Exercises } = require('../models');

const exercisesData = [
  {
    title: 'Swiss Ball Crunch',
    artist: 'trainer1',
    exhibition_date: 'March 30, 2018',
    Workout_id: 1,
    filename: 'swissballcurl.PNG',
    description:
      'Lie back on the Swiss ball. Your lower back should be supported by the ball.Curl your body up off the ball until sitting',
  },
  {
    title: 'Ab',
    artist: 'trainer2',
    exhibition_date: 'May 05, 2017',
    Workout_id: 1,
    filename: '02-cosmos-flowers.jpg',
    description: 'Pink cosmos flowers against a blue sky.',
  },
  {
    title: 'Standing overhead dumbbell presses',
    artist: 'trainer3',
    exhibition_date: 'June 10, 2019',
    Workout_id: 2,
    filename: 'overheadpress.PNG',
    description: 'Move the weights overhead so your upper arms are parallel to the floor. Bracing your core, begin to push up until arms are fully extended above head.',
  },
  {
    title: 'Beach Chairs',
    artist: 'trainer4',
    exhibition_date: 'July 4, 2020',
    Workout_id: 2,
    filename: '04-beach-chairs.jpg',
    description: 'Two beach chairs under a beach umbrella on the beach.',
  },
  {
    title: 'Beach Sunrise',
    artist: 'trainer5',
    exhibition_date: 'August 14, 2016',
    Workout_id: 2,
    filename: '05-beach-sunrise.jpg',
    description: 'Sun setting in the horizon with waves lapping the shore.',
  },
  {
    title: 'Lunges',
    artist: 'trainer6',
    exhibition_date: 'October 15, 2018',
    Workout_id: 3,
    filename: 'lunges.jpg',
    description:
      'Take a step forward with your right leg and bend your right knee as you do so, stopping when your thigh is parallel to the ground.',
  },
  {
    title: 'Autumn Mountains',
    artist: 'trainer7',
    exhibition_date: 'November 3, 2016',
    Workout_id: 3,
    filename: '07-autumn-mountains.jpg',
    description:
      'Mountains with red and yellow leaves against a background of hazy rolling hills.',
  },
  {
    title: 'Burpees',
    artist: 'trainer8',
    exhibition_date: 'December 24, 2020',
    Workout_id: 4,
    filename: 'burpee.PNG',
    description:
      'With your hands out in front of you, start to squat down. When your hands reach the ground, pop your legs straight back into a pushup position. Jump your feet up to your palms by hinging at the waist.',
  },
  {
    title: 'Winter Home',
    artist: 'trainer9',
    exhibition_date: 'January 20, 2018',
    Workout_id: 4,
    filename: '09-winter-home.jpg',
    description:
      'Log cabin blanketed in heavy white snow with tall snow covered pine trees in the background.',
  },
];

const seedExercises = () => Exercises.bulkCreate(exercisesData);

module.exports = seedExercises;
